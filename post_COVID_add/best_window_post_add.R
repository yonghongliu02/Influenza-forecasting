...
for (model in models) {
  for (window_size in window_intervals) {
    ...
    if (file.exists(path_)) {
      ...
      ##删除窗口
      df=df[which(df$date>=as.Date(yearweek("2024 W27"))&df$date<=as.Date(yearweek("2025 W26"))),]
...
# 保存结果
write.csv(optimal_windows, file = '../../Results/Interval_ydiff_raw/total_interval_window_2023_add.csv', row.names = FALSE)
