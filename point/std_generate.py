...
for window_size in [5, 8, 10, 12, 14, 16, 18, 20, 
      ...
      date_list_o = pd.date_range('2016-07-04','2020-06-28',freq='W-MON')
          ...
          for wi in range(9):# 遍历每个预测步长（0~8周）
             ...
             # date_list = [idate for idate in date_list_o if idate in df_w.date.unique()]
              ...
              for di in date_list:# 遍历每个预测日期
                    ...
                    # 提取滑动窗口内的历史数据
                    df_wd = df_w.loc[(df_w.date>di-timedelta(days = 7*window_size))&
                                     ((df_w.date<=di)),:]
                    df_wd['ydiff'] = df_wd['point'] - df_wd['true']###？？？？？应改为 df_wd['point'] - df_wd['true']，否则会因索引对齐问题导致计算错误。
                    # 计算预测误差的标准差
                    sd = df_wd.ydiff.std()
                    # 生成正态分布的分位数
