...
AVG_ENSEMBLE <- function(model_list, forecasting_mode, origin_path, rolling_start_date) {
  ...
  self$SAE <- function() {
    ...
    for (m in self$model_list) {
      ...
      df_mt_o <- df_mt_o %>% filter(date > self$rolling_start_date, date <= as.Date('2024-06-24'))
      df_mt_o$model <- m
    ...
    date_analysises <- date_analysises[date_analysises >= as.Date('2015-10-06') & date_analysises <= as.Date('2024-06-24')]
...
rolling_start_date <- as.Date('2015-07-06')
decay_mode <- 6
