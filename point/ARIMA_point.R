...
# 计算每个process_date的统计量
result <- data_rt %>%
  group_by(date_analysis) %>%
  summarise(
    min_value_date = min(date, na.rm = TRUE),
    max_value_date = max(date, na.rm = TRUE),
    #num_value_dates = n_distinct(rate),
    .groups = "drop"
  )
...
covar_all<-c("mean_temperature", "rh")
remove_last_n <- 1
...
res11=res11[-which(res11$date>as.Date(yearweek("2020 W27"))&
                     res11$date<as.Date(yearweek("2023 W26"))),]
