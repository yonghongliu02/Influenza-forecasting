...
for (iDate in seq_along(dates_analysis)){
  ...
  for (iRegion in seq_along(regions)) {
    ...
    # Fit model 
    # modelspec <- ugarchspec(mean.model = list(armaOrder=c(mas,0) ,external.regressors  = as.matrix(sub[,c(2:ncol(sub))])), 
    #                         variance.model = list(model = "sGARCH",garchOrder= c(mgs,1)))
    ...
    # 提取系数
    re.coef <- as.data.frame(coef(mod))
    raw_names <- rownames(re.coef)
    
    # 获取真实协变量名（sub 除了 iHosp 以外的列）
    xreg_names <- colnames(sub)[2:ncol(sub)]  # 注意 sub[,1] 是 iHosp
    
    # 替换 mxregX 为真实变量名
    mapped_names <- sapply(raw_names, function(x) {
      if (grepl("^mxreg[0-9]+$", x)) {
        idx <- as.numeric(gsub("mxreg", "", x))
        if (idx <= length(xreg_names)) {
          return(xreg_names[idx])
        } else {
          return(x)
        }
      } else {
    ...
    re.coef <- tibble(
      ...
      feature_coef = re.coef[[1]],
      date = dates_analysis[iDate]
    )
    
    # 提取 predictor 和 lag 信息（可选）
    re.coef <- re.coef %>%
      mutate(
        predictor = gsub("[^a-zA-Z_]", "", feature_name),
        lag_order = str_extract(feature_name, "[0-9]+$"),
        lag_order = if_else(predictor %in% c("monthid", "weekid"), "1d", paste0(lag_order, "d"))
      )
...
res_filtered <- res %>%
  filter(!predictor %in% c("mu","omega","alpha","beta")) 
