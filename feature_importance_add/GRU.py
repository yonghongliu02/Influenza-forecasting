...
def one_rolling(df, test_start, test_end, pred_horizon, exp_mode=True):
    ...
    for horizon_idx in range(pred_horizon):
        ...
        # 反向传播
        # 最后一个步长不需要retain_graph
        retain_graph = (horizon_idx < pred_horizon - 1)
        pred_sum.backward(retain_graph=retain_graph)
        ...
        # 去掉最后pred_horizon个样本（避免边界效应，参考第一个文件逻辑）
